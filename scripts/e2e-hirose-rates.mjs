...
try {
  ...
  const history = await page.evaluate(() => ({
    ...
    stored: JSON.parse(localStorage.getItem('dollar-to-lira:v1') || '{}').daily?.find((row) => row.date === '2026-09-04') || null
  }));

  assert.equal(history.start, '2026-07-10', `unexpected rate history start: ${history.start}`);
  assert.equal(history.end, '2026-09-04', `unexpected rate history end: ${history.end}`);
  assert.equal(history.records, 41, `unexpected rate history record count: ${history.records}`);
  ...
  assert.equal(history.stored?.rate, 48.4438, 'historical USDTRY row was not backfilled');
  assert.equal(history.stored?.usdJpy, 156.12, 'historical USDJPY row was not backfilled');
  assert.equal(history.stored?.rateSource, 'hirose-ask-23close', 'historical row source marker is missing');
  console.log('41 Hirose 23:00 ASK closes + local backfill: PASS');
  ...
  assert.equal(Number(await page.locator('#dailyRate').inputValue()), 48.4438, 'USDTRY past form auto-fill is wrong');
  assert.equal(Number(await page.locator('#dailyUsdJpy').inputValue()), 156.12, 'USDJPY past form auto-fill is wrong');
  console.log('historical date form auto-fill: PASS');
  ...
  await page.evaluate(() => {
    ...
    const row = saved.daily.find((item) => item.date === '2026-09-04');
    row.rate = 49.9999;
    row.usdJpy = 150.123;
